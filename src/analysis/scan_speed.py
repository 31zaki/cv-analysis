...
def run(data_dir, baseline_points, device_filter=None, apply_smoothing=True,
    ...
    for fname in dta_files:
        ...
        c   = smooth(np.asarray(c_raw, dtype=float)) if apply_smoothing \
    ...
    figs = _make_figures(records)

    if base_output_dir:
        dev = _sanitize(records[0]["device"])
        out = os.path.join(base_output_dir, _sanitize(experiment_type), dev)
        os.makedirs(out, exist_ok=True)
        stems = ["all_cv_raw", "all_cv_baseline_sub", "peak_vs_scanrate"]
        for (fig, _), stem in zip(figs, stems):
            save_for_paper(fig, os.path.join(out, f"{dev}_{stem}.png"))

    return {"records": records, "skipped": skipped, "figures": figs}


def _make_figures(records):
    apply_mpl_style()
    nc = len(PLOT_COLORS)
    figs = []
    ...
    for i, r in enumerate(records):
        ax2.plot(r["diff"], color=PLOT_COLORS[i % nc],
                 linewidth=1.6, label=f"{r['rate']} mV/s")
    ...
    ax3.plot(rates, ox_v,  "o", color=PLOT_COLORS[0], label="Oxidation",  markersize=7)
    ax3.plot(rates, red_v, "s", color=PLOT_COLORS[1], label="Reduction",  markersize=7)
    ax3.plot(rates, np.poly1d(np.polyfit(rates, ox_v,  1))(rates),
             "--", color=PLOT_COLORS[0], lw=1.2, alpha=0.7)
    ax3.plot(rates, np.poly1d(np.polyfit(rates, red_v, 1))(rates),
             "--", color=PLOT_COLORS[1], lw=1.2, alpha=0.7)
